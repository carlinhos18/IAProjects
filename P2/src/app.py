...
def load_artifacts():
    ...
    rf_importance = joblib.load(MODEL_DIR / "rf_feature_importance.pkl")
    gb_importance = joblib.load(MODEL_DIR / "gb_feature_importance.pkl")
    lr_importance = joblib.load(MODEL_DIR / "lr_feature_importance.pkl")

    return rf_model, gb_model, lr_model, encoder, metrics, rf_importance, gb_importance, lr_importance
...
@st.cache_data
def load_data():
    df = pd.read_csv("../datasets/Games.csv")#Não é preciso dar load do GameFake pq ele usa as mesmas equipas
...
best_model = metrics.sort_values(by="Accuracy", ascending=False).iloc[0]
st.success(
    f"{best_model['Model']}  is the best ({best_model['Accuracy']:.2%} accuracy)"
)
...
selected_model = st.selectbox(
    "Choose Prediction",
    ["Random Forest", "Gradient Boost", "Logistic Regression"],
)
...
st.subheader(f"Top Features :{selected_model}")
...
if home and away:
    ...
    with c1:
        st.metric("Last 10 ", f"{h2h_wins}–{h2h_total - h2h_wins}")
    with c2:
...
if st.button("Predict Winner", use_container_width=True, type="primary"):
    ...
    # Encoding das equipas
    team_input    = pd.DataFrame([{'hometeamName': home, 'awayteamName': away}])
    ...
    insights_df = pd.DataFrame({
        "Metric": ["Win Rate", "Last 5 games", "Current Win Streak", "ELO :"],
        home: [f"{home_wr:.1%}", f"{home_form:.1%}", f"{home_streak_val}", f"{home_elo:.0f}"],
        away: [f"{away_wr:.1%}", f"{away_form:.1%}", f"{away_streak_val}", f"{away_elo:.0f}"],
    })
    ...
    if h2h_total > 0:
        st.divider()
        st.subheader("Last 10 games between these teams")
        st.write(
            f"**{home}** won **{h2h_wins}** of their last "
            f"{h2h_total} matchups against **{away}**"
        )
    else:
